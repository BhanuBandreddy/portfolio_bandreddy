...
export default async function ProjectPage({params}:{params:Promise<{slug:string}>}){
  ...
  return <PageShell>
    ...
    <section className="case-body">
      <article><span className="section-index">01 · PROBLEM TO RESOLVE</span><h2>What prevented the workflow from operating as one case.</h2><p>{project.challenge}</p></article>
    ...
    <section className="capability-section"><div><span className="section-index">03 · INCLUDED CAPABILITIES</span><h2>The functions contained in the solution.</h2></div><div className="capability-grid">{project.capabilities.map((c,i)=><span key={c}><b>0{i+1}</b>{c}</span>)}</div></section>
